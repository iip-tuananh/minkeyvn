...
class BlogController extends Controller
{
    public function list()
    {
        ...
        ->paginate(12);
        $data['news'] = Blog::where(['status'=>1, 'home_status'=>1])->orderBy('id', 'desc')->limit(10)->get(['id', 'title', 'slug', 'image','category']);
    ...
    {
        $data['news'] = Blog::where(['status'=>1])->orderBy('id', 'desc')->limit(5)->get(['id', 'title', 'slug', 'image','category']);
        $data['discountPro'] = Product::where('status', 1)->where('discount', '>', 0)->limit(10)->get(['id', 'name', 'price', 'discount', 'images', 'cate_slug', 'type_slug', 'slug']);
